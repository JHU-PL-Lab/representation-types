...
module Functor_Util (F : Functor) = struct
  ...
  module Syntax = struct
    ...
    (** Replace the value inside a functor *)
    let ( <$ ) a f = F.map (fun _ -> a) f
    let ( $> ) f a = F.map (fun _ -> a) f
...
module Alternative_Util (A : Alternative) = struct
  ...
  (** Shorter way to reduce over a list using the monoidal structure. *)
  let one_of (fas : 'a t list) : 'a t =
    List.fold_left append empty fas
...
(**
  A monad representing non-deterministic
  computations stateful both globally and locally,
  with individual concurrent threads and a
  log-writing facility.
*)
module NCoWS
  (S : sig type t end) (* global state *)
  (T : sig type t end) (* local state  *)
  (W : Monoid)         (* monoidal log *)
: sig
  ...
  (**
    A thread of concurrent excecution.
    Any particular thread is either fully
    completed executing, yielded to other
    executing threads and waiting to resume,
    or somehow failed during runtime.

    Note that threads do not communicate,
    and so this mechanism does not need
    to be as sophisticated as a coroutine.
    Each thread merely proceeds on its own,
    and potentially forks, but otherwise behaves
  ...
  *)
  ...
  (**
    The monadic type. Composes a writable global log {!W}
    with a mutable global state {!S} and thread-local
    states {!T}. Nondeterminism is represented by threads
    forking and returning different results in each child.
  *)
  and 'a t = S.t -> T.t ->
    (S.t * W.t * (T.t * 'a thread) list)
  ...
  let rec map f m s t =
    let (s', w, tas) = m s t in
    let tbs = tas |> List.map
      (fun (t, a) -> (t, map_thread f a))
    in
    (s', w, tbs)
  ...
  and map_thread f =
    function
    | Result a -> Result (f a)
    | Yield ma -> Yield (map f ma)
    | Failed e -> Failed e


  let pure (a : 'a) : 'a t =
    fun s t ->
      (s, W.empty, [(t, Result a)])

  let empty : 'a t =
    fun s _t ->
      (s, W.empty, [])

  let append (m1 : 'a t) (m2 : 'a t) : 'a t =
    fun s t ->
      let (s',  w1, t1s) = m1 s  t in
      let (s'', w2, t2s) = m2 s' t in
      (s'', W.append w1 w2, t1s @ t2s)

  let rec join (mma : 'a t t) : 'a t =
    fun s t ->
      let open Util.Diff_list in
      let rec run_threads s w tmas tas =
        match tmas with
        | [] -> (s, w, tas)
        | tma :: tmas ->
        match tma with
        | t, Result ma ->
            let (s', w', tas') = ma s t in
            run_threads s' (W.append w w') tmas (tas' @ tas)

        | t, Yield mma ->
            run_threads s w tmas ((t, Yield (join mma)) :: tas)

        | t, Failed e ->
            run_threads s w tmas ((t, Failed e) :: tas)
      in
      let (s1, w1, tmas) = mma s t in
      run_threads s1 w1 tmas []
...
= struct
  ...
  module NCoWS = NCoWS(S)(T)(W)
  
  open struct 
    module Monad_Util = Monad_Util(NCoWS)
    module Alternative_Util = Alternative_Util(NCoWS)
  end

  include NCoWS
  include Monad_Util
  include Alternative_Util

  module Syntax = struct
    include Monad_Util.Syntax
    include Alternative_Util.Syntax
  end
  ...
  let yield : unit t =
    fun s t ->
      (s, W.empty, [(t, Yield (pure ()))])

  let pures : 'a list -> 'a t =
    fun xs s t ->
      (s, W.empty, List.map (fun x -> (t, Result x)) xs)

  let of_option : 'a option -> 'a t =
    function
    | Some(x) -> pure x
    | None    -> empty
  ...
  let raise : exn -> 'a t =
    fun e s t ->
      (s, W.empty, [(t, Failed e)])

  let catch (handle : exn -> 'a t) : 'a t -> 'a t =
    fun ma ->
      fun s t ->
      let (s, w, tas) = ma s t in
      let tas' = tas |> List.map
        (function
        | t, Failed e -> t, Yield (handle e)
        | other -> other
        )
      in (s, w, tas')
  ...
  let censor f (ma : 'a t) : 'a t =
    fun s t ->
      let (s', w, tas) = ma s t in
      (s', f w, tas) 

  let listen f ma =
    censor (fun w -> f w; w) ma

  let get : S.t t =
    fun s t ->
      (s, W.empty, [(t, Result s)])

  let modify (f : S.t -> S.t) : unit t =
    fun s t ->
      (f s, W.empty, [(t, Result ())])

  (**
    Actually, in this setting, this operator
    might not be very meaningful...
    I'm not sure that this will really achieve anything.
    Fortunately I don't need it. Perhaps [control]-like
    primitives only make sense for thread-local state.
  *)
  let control f f' (ma : 'a t) : 'a t =
    fun s t ->
      let (s', w, tas) = ma (f s) t in
      (f' s s', w, tas)

  let get' : T.t t =
    fun s t ->
      (s, W.empty, [(t, Result t)])

  let modify' (f : T.t -> T.t) : unit t =
    fun s t ->
      (s, W.empty, [(f t, Result ())])
  ...
  (**
    An evaluator which sequences computations which {!Yield}ed
    after the results in each iteration, so that long-running
    computations which intermittently yield can be interleaved,
    and results can incrementally be accumulated.
  *)
  let eval (ma : 'a t) s t : (S.t * W.t * 'a list * exn list) =
    let module DL = Util.Diff_list in
    let rec eval_all s w tas rs fs =
      match tas with
      | [] -> (s, w, DL.to_list rs, DL.to_list fs)
      | ta :: tas ->
      match ta with
      | _, Result a -> eval_all s w tas (DL.snoc rs a) fs
      | _, Failed e -> eval_all s w tas rs (DL.snoc fs e)
      | t, Yield ma ->
          let (s', w', tas') = ma s t in
          eval_all s' (W.append w w') (tas @ tas') rs fs
    in
    let (s, w, tas) = ma s t in
    eval_all s w tas DL.nil DL.nil
          

  (**
    Run a sort of evaluator {i internal} to the computation,
    such that the complete list of results is visible to
    the caller. This function yields so as to ensure fair evaluation
    of itself among other ongoing threads, and lifts writer and
    exception-related actions into the outer environment.
    If the computation [ma] never terminates, then neither can
    [collect ma], of course.

    Note that because we have no general way to
    combine values of type {!T.t}, we cannot witness local thread
    state changes from outside. However, of course each of the
    threads whose results we're collecting have terminated, so it
    is not very meaningful anyway.

    (Rather, if it {b is} meaningful, the computation should
    return this value using {!get'} instead of leaving it implicit).
  *)
  let collect (ma : 'a t) : 'a list t =
    let module DL = Util.Diff_list in
    let rec eval_all tas rs : 'a list t = 
      match tas with
      | [] -> pure (DL.to_list rs)
      | ta :: tas ->
      match ta with
      | _,  Result a -> eval_all tas (DL.snoc rs a)
      | _,  Failed e -> raise e <|> eval_all tas rs
      | t', Yield ma -> yield *>
        fun s t ->
          let (s', w, tas') = ma s t' in
          (tell w *> eval_all (tas @ tas') rs) s' t 
    in
    fun s t ->
      let (s, w, tas) = ma s t in
      (tell w *> eval_all tas DL.nil) s t

  (**
    Ensures that the value received from
    the monadic action is unique among
    all nondeterministic results.

    Note that as a result, the thread-local
    state changes made by [ma] cannot be recovered.
    Ideally this should be a short action which only
    reads state.
  *)
  let unique (ma : 'a t) : 'a t =
    let* all = collect ma in
      all
      |> List.sort_uniq compare
      |> pures

  (**
    Ignores a computation without allowing
    that computation to split our current execution
    into several identical ones transparently,
    but while still letting it have effects on
    the global state.
  *)
  let ignore (ma : 'a t) : unit t =
    map ignore (collect ma)
